...
Future<void> generateAbcencesPdf(
  ...
  List<Student> students = studentService.getAll().where((element) => attendance.absentStudent!.contains(element.id)).toList();
  List<Prof> profsList = profService.getAll().where((element) => attendance.absentProf!.contains(element.id)).toList();
  ...
  final String title = '${translation(cont).absent} :   ${translation(cont).total }   ${students.length + profsList.length }';
